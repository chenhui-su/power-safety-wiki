...
**内容策划组（无仓库权限）**：请阅读 [getting-started.md](getting-started.md) 完成本地写作、本地预览和截图、素材打包提审。
...
2. 🎯 在「贡献记录表」中认领事故主题
3. 📝 本地完成案例文档，建议进行本地预览和截图
4. 🎬 使用 NotebookLM 生成分析视频
5. 📦 将 `项目文件 + 视频文件` 提交给内容审核组；如已按建议进行本地预览和截图，可一并附上截图
6. ✅ 内容审核组审核通过后，由技术组完成视频上传、页面入库与部署

> 💡 **无需编程经验**：你只需要完成内容撰写与素材打包，内容审核组和技术组会协助完成审核与上线。
