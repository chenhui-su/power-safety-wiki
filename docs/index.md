# 电力系统大停电事故案例库

<div style="text-align: center; margin: 2rem 0;">
  <p style="font-size: 1.2rem; color: #666;">
    电力系统安全分析领域权威知识平台<br>
    <strong>上海交通大学 130 周年校庆献礼</strong>
  </p>
</div>

---

## 🎯 项目简介

电力系统是现代社会的命脉，大停电事故往往造成巨大的经济损失和社会影响。本案例库系统收集、整理全球重大停电事故，通过专业分析和 AI 辅助解读，为电力系统安全研究提供宝贵参考。

**我们的目标**：做电力系统安全分析方向最好的中文网站。

!!! info "当前阶段说明"
    当前仓库处于协作启动阶段，现有事故页均为占位稿，内容待持续补充完善。

---

## 📚 事故案例

<div class="grid cards" markdown>

-   :material-lightning-bolt:{ .lg .middle } **北美地区**

    ---

    - [2003年美加大停电](incidents/2003-us-canada/index.md)（占位） - 待完善
    - [1965年美国东北大停电](incidents/1965-us-northeast/index.md)（待收录） - 占位页，欢迎认领完善

-   :material-lightning-bolt:{ .lg .middle } **欧洲地区**

    ---

    - [2003年意大利大停电](incidents/2003-italy/index.md)（待收录） - 占位页，欢迎认领完善
    - [2006年欧洲大停电](incidents/2006-europe/index.md)（待收录） - 占位页，欢迎认领完善

-   :material-lightning-bolt:{ .lg .middle } **亚洲地区**

    ---

    - [2012年印度大停电](incidents/2012-india/index.md)（占位） - 待完善

-   :material-plus-circle:{ .lg .middle } **更多案例**

    ---

    持续更新中，[查看完整案例索引](incidents/index.md)

</div>

---

## 🤖 AI 辅助分析

每个事故案例都配备了 AI 辅助分析视频，利用 NotebookLM 等先进工具，将复杂的技术报告转化为易于理解的对话式解读。

<div style="background: #f8f9fa; padding: 1.5rem; border-radius: 8px; margin: 1rem 0;">
  <strong>🎬 特色功能</strong>
  <ul>
    <li>基于官方调查报告的深度解读</li>
    <li>对话式呈现，通俗易懂</li>
    <li>技术原理可视化讲解</li>
    <li>经验教训总结提炼</li>
  </ul>
</div>

---

## 🤝 参与贡献

我们欢迎所有对电力系统安全感兴趣的同学参与内容贡献！

!!! tip "如何参与"
    1. 📖 阅读 [贡献指南](https://gitee.com/ch_su/power-safety-wiki/blob/main/CONTRIBUTING.md)
    2. 🎯 认领感兴趣的事故主题
    3. 📝 按模板完成文档，建议进行本地预览和截图
    4. 🎬 使用 NotebookLM 生成分析视频
    5. 📦 打包提交给内容审核组
    6. ✅ 技术组完成视频入库与页面部署

> **贡献者署名永久保留**，共同打造交大在电力安全分析领域的学术 IP。

---

## 👥 团队

本项目由上海交通大学电力系统安全分析课程发起，感谢所有贡献者的付出。

[了解更多 →](about.md)

---

<div style="text-align: center; margin-top: 3rem; padding: 2rem; background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); border-radius: 12px; color: white;">
  <h3 style="margin: 0; color: white;">🎓 献礼上海交通大学 130 周年校庆</h3>
  <p style="margin: 0.5rem 0 0 0; opacity: 0.9;">2026年4月6日</p>
</div>
