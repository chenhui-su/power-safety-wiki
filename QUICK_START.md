...
- `getting-started.md`（仓库根目录）：面向内容策划组，负责本地写作、本地预览和截图、视频制作与提审
...
# 4. 接收内容审核组审核通过的材料（MD + 可选预览截图 + 视频）
# 5. 将材料中的视频发视频服务器维护成员，获取其提供的视频链接
# 5. 将视频链接嵌入对应 index.md，将文件按规范移动到对应位置，并将路径添加到 mkdocs.yml 的导航页面
# 6. 本地预览验证
mkdocs serve
...
> - 内容策划组提审材料至少包含：项目文件、视频文件；如已按建议进行本地预览和截图，可附上截图
